...
#ifndef TYR_PLANNING_SPARSE_NODE_HPP_
#define TYR_PLANNING_SPARSE_NODE_HPP_
...
{
public:
    SparseNode() noexcept : m_task(nullptr), m_state_metric(), m_state_index(StateIndex::max()) {}
    SparseNode(StateIndex state_index, float_t state_metric, Task& task) noexcept : m_task(&task), m_state_metric(state_metric), m_state_index(state_index) {}

    SparseState<Task> get_state() const { return m_task->get_state(m_state_index); }
...
template<typename Task>
using SparseNodeList = std::vector<SparseNode<Task>>;
}
