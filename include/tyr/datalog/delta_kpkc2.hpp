...
public:
    ...
    Anchor operator[](size_t anchor) const noexcept
    {
    ...
    void sort(std::vector<size_t>& out_order) const noexcept
    {
        out_order.resize(size());
        std::iota(out_order.begin(), out_order.end(), size_t(0));
...
private:
    /// @brief Return true iff the given set of vertex form a clique in the full graph.
    /// @param vertices is the given set of vertices.
    /// @return true iff there is an edge between every pair of vertices, and otherwise, false.
    bool is_clique(const Anchor& anchor) const;

    /// @brief Return true iff either one of the vertices was added or one of their adjacent edges was added.
    /// @param vertices  is the given set of vertices.
    /// @return true iff either one of the vertices was addred or one of their adjacent edges was added.
    bool is_delta(const Anchor& anchor) const;

    void seed_from_anchor(const Anchor& anchor, Workspace& workspace) const;
...
void DeltaKPKC::for_each_k_clique(Callback&& callback, Workspace& workspace, const Anchor& anchor) const
{
    ...
    if (!is_delta(anchor))
        return;  ///< The anchors do not contain added ajacent graph structures.

    if (!is_clique(anchor))
        return;  ///< The anchors do not form a clique then there cannot be a k-clique after all.

    seed_from_anchor(anchor, workspace);
