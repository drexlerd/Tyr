...
public:
    using Variant = Term::Variant;

    TermProxy(const Repository& repo, Term term) : m_repo(&repo), m_term(term) {}

    const Variant& index_variant() const noexcept { return m_term.value; }
    const Repository& context() const noexcept { return *m_repo; }
    ...
    decltype(auto) visit(F&& f) const
    {
        return std::visit(
            [&](auto&& idx) -> decltype(auto)
            {
                ...
                return std::forward<F>(f)(Proxy(*m_repo, idx));
            },
            m_term.value);
    }
};
