...
ground_planning(View<Index<ConjunctiveCondition>, C_SRC> element, View<IndexList<Object>, C_DST> binding, Builder& builder, C_DST& destination)
{
    ...
    for (const auto literal : element.template get_literals<FluentTag>())
        conj_cond.fluent_literals.push_back(ground_planning(literal, binding, builder, destination).get_data());
...
ground_planning(View<Index<ConjunctiveEffect>, C_SRC> element, View<IndexList<Object>, C_DST> binding, Builder& builder, C_DST& destination)
{
    // Fetch and clear
...
ground_planning(View<Index<ConditionalEffect>, C_SRC> element, View<IndexList<Object>, C_DST> binding, Builder& builder, C_DST& destination)
{
    // Fetch and clear
    ...
    cond_effect.effect = ground_planning(element.get_effect(), binding, builder, destination).get_index();
        ...
        for_element_in_cartesian_set(parameter_domains.begin(),
                                     ...
                                     [&](auto&& binding_cond)
                                     {
                                         ...
                                         action.effects.push_back(
                                             ground_planning(cond_effect, make_view(binding_full, binding.get_context()), builder, destination).get_index());
                                     });
    }
